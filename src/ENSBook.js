...
class ENSBook extends React.Component {
  ...
  updateBalance = async () => {
    this.setState({
      balance: utils.formatEther(await this.state.signer.getBalance()),
    });
  };
  ...
  registerName = async (label, _duration, receiver, regFrom = 0, regTo = 3) => {
    if (regFrom >= regTo) {
    ...
    const regFrom0 = async () => {
      ...
      const commitOverrides = {}; // config overrides
      if (conf.custom.wallet.gasPrice > 0) {
      ...
      try {
        ...
        const commitTx = await ETHRegCtrlCon.commit(
          regInfo.commitment,
          commitOverrides
        );
    ...
    const regFrom05 = async () => {
      const commitTxLink =
        `<a href="${confFixed.scanConf[network]}tx/${regInfo.commitTxHash}" target="_blank" rel="noreferrer">${t('c.tx')}</a>`;
      ...
      const commitTxReceipt = await provider.waitForTransaction(
    ...
    const regFrom2 = async () => {
      ...
      regOverrides.gasLimit = conf.custom.register.registerWithConfig
        ? 300000
        : 220000;
      regOverrides.value = (await ETHRegCtrlCon.rentPrice(label, duration))
        .mul(105)
        .div(100);
    ...
    const regFrom25 = async () => {
      const regTxLink =
        `<a href="${confFixed.scanConf[network]}tx/${regInfo.regTxHash}" target="_blank" rel="noreferrer">${t('c.tx')}</a>`;
      ...
      const regTxReceipt = await provider.waitForTransaction(
        regInfo.regTxHash,
        2
      ); // waitConfirms: 2
  ...
  renewName = async (label, _duration) => {
    ...
    renewOverrides.value = (await ETHRegCtrlCon.rentPrice(label, duration))
      .mul(105)
      .div(100);
    ...
    try {
      ...
      renewTxLink =
        `<a href="${confFixed.scanConf[network]}tx/${renewTx.hash}" target="_blank" rel="noreferrer">${t('c.tx')}</a>`;
  ...
  renewNames = async (renewList, _duration) => {
    const { provider, signer, network } = this.state;
    ...
    renewsOverrides.value = (await BulkRenewCon.rentPrice(renewList, duration))
      .mul(105)
      .div(100);

    try {
      renewsTx = await BulkRenewCon.renewAll(
        renewList,
        duration,
        renewsOverrides
      );
      renewsTxLink =
        `<a href="${confFixed.scanConf[network]}tx/${renewsTx.hash}" target="_blank" rel="noreferrer">${t('c.tx')}</a>`;
  ...
  estimateCost = async (label, _duration) => {
    const { provider, network } = this.state;
  ...
  subscribeProvider = async (provider) => {
    ...
    provider.on('chainChanged', async (chainId) => {
      ...
      if (isSupportedChain(Number.parseInt(chainId, 16))) {
